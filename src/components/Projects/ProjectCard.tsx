    ...
    return (
        ...
        >
            <div className='project-img col-sm-12 col-md-12'>
                ...
                {project.name == "E-commerce app" || project.name == "Savage Blog"|| project.name == "Savage Files"|| project.name == "SCADATECH"
        ...
        <div className='container-fluid'>
            <div className='row'>
                <div className='project-holder'>
                    {AllProjects.filter((el) => el.name !== "E-commerce app" && el.name !== "Savage Blog" && el.name !== "Savage Files"  && el.name !== "LearnHub" && el.name !== "SCADATECH"  ).map((el) => (
            ...
            <div className='row'>
                <div className='project-holder'>
                    {AllProjects.filter((el) => el.name !== "Agency app" && el.name !== "Weather app"  && el.name !== "Savage Files"  && el.name !== "SCADATECH" && el.name !== "LearnHub" && el.name !== "Magasin").map((el) => (
                        <Project key={el.name} project={el} arrowImg={arrow} />
                    ))}
                </div>
            </div>
            <div className='row'>
                <div className='project-holder'>
                    {AllProjects.filter((el) => el.name !== "Agency app" && el.name !== "Weather app" && el.name !== "E-commerce app" && el.name !== "Savage Blog" && el.name !== "Magasin" ).map((el) => (
                        <Project key={el.name} project={el} arrowImg={arrow} />
                    ))}
                </div>
            </div>
